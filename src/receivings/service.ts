...
export class ReceivingsService {
  ...
  constructor(
    ...
    private receivingModel: Model<ReceivingDocument>,
  ) {}

  async create(createReceivingDto: CreateReceivingDto): Promise<Receiving> {
    const client = await this.clientsService.findById(
      createReceivingDto.client,
    );

    if (!client)
      throw new ClientNotDefinedException('Client with this id is not defined');
  ...
  async findAll(): Promise<Receiving[]> {
    return (await this.receivingModel.find().populate('client').exec()).sort(
  ...
  async findByRangeAndClient(
    start: number,
    end: number = Date.now(),
    client: string = null,
  ) {
    return (
      await this.receivingModel
      ...
      .sort((r1, r2) =>
        r1.client.name
          .toLowerCase()
          .localeCompare(r2.client.name.toLowerCase()),
      )
  ...
  async update(updateReceivingDto: UpdateReceivingDto): Promise<Receiving> {
    return this.receivingModel
      .findOneAndUpdate(
        { _id: new Types.ObjectId(updateReceivingDto.id) },
        {
          ...updateReceivingDto.newData,
          ...(updateReceivingDto.newData.records?.length
            ? {
                totalWeight: updateReceivingDto.newData.records.reduce(
                  (acc, curr) => acc + curr.weight,
                  0,
                ),
                totalPrice: +updateReceivingDto.newData.records
                  .map(({ weight, price }) => weight * price)
                  .reduce((acc, curr) => acc + curr)
                  .toFixed(0),
              }
